...
export function ChatComposer({
  ...
  const confirmSkip = useCallback(async () => {
    disarmSkip();
    toast.message("Finding a new stranger…");
    await onSkip();
  }, [disarmSkip, onSkip]);
  ...
  }, [disabled, skipArmed, confirmSkip, armSkip]);
  ...
  return (
    <div
      className={cn(
        "absolute bottom-16 left-0 z-20 w-full bg-gradient-to-t from-background via-background/95 to-transparent p-4 md:p-6 lg:bottom-0",
        ...
        <div className="flex items-end gap-2 rounded-full border border-outline-variant/40 bg-surface-container/80 p-1.5 pl-2 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-xl transition-all focus-within:border-magenta/50 focus-within:shadow-[0_0_28px_rgba(232,121,249,0.2)] focus-within:ring-1 focus-within:ring-magenta/30">
          <Popover>
            ...
            >
              ...
              <button
                ...
                onClick={() => {
                  if (isGuest()) {
                    toast.info("Create an account to share GIFs.");
                    return;
                  }
                  const gif =
                    SAMPLE_GIFS[Math.floor(Math.random() * SAMPLE_GIFS.length)]!;
                  onSend("gif", "gif", gif);
                }}
              >
                <Film className="size-4 text-secondary" /> Share a GIF
              </button>
            </PopoverContent>
          </Popover>
          ...
          >
            ...
            <button
              ...
              aria-label={skipArmed ? "Confirm skip" : "Skip stranger"}
              aria-pressed={skipArmed}
            >
              {skipArmed ? "confirm?" : "Skip"}
            </button>
          </div>

  ...
  const handleResendOtp =
    async () => {
      ...
      const handleGuest = () => {
        ...
        toast.message(
          "Browsing as guest ΓÇö some features are limited."
        );

        router.push("/chats");
      };

      if (guestParam === "1" && !session) {
        return (
          <div className="flex min-h-[300px] items-center justify-center">
            <div className="flex flex-col items-center gap-3">
              <span className="size-6 animate-spin rounded-full border-2 border-magenta border-t-transparent" />
              <p className="text-xs text-on-surface-variant">Entering the lounge…</p>
            </div>
          </div>
        );
      }

...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-xl border border-transparent bg-clip-padding text-sm font-semibold whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:scale-[0.98] disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "bg-gradient-to-r from-primary to-magenta text-white shadow-[0_4px_16px_rgba(232,121,249,0.3)] hover:opacity-90 hover:shadow-[0_6px_22px_rgba(232,121,249,0.4)]",
        outline:
          "border-outline-variant/50 bg-surface-container/40 hover:border-primary/40 hover:bg-muted hover:text-foreground aria-expanded:bg-muted aria-expanded:text-foreground",
        secondary:
          "bg-secondary/90 text-secondary-foreground shadow-[0_2px_12px_rgba(34,211,238,0.25)] hover:bg-secondary aria-expanded:bg-secondary aria-expanded:text-secondary-foreground",
        ghost:
...
function Button({
  ...
  // When composing with non-button elements (e.g. Next.js Link), disable
  // native button expectations so Base UI doesn't warn about semantics.
